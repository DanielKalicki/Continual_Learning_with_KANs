# from https://github.com/yuedajiong/minimum-kan/blob/main/kan.py
import torch
...
class KALayer(torch.nn.Module):
    def __init__(self, in_dim, out_dim, grid_number=5, k=3, noise_scale=0.1, scale_base=1.0, scale_spline=1.0, base_fun=torch.nn.SiLU(), grid_eps=0.02, grid_range=[-1, +1], sp_trainable=True, sb_trainable=True):
        def curve2coef(y_eval, x_eval, grid, k): #converting B-spline curves to B-spline coefficients using least squares.  # x_eval: (size, batch); y_eval: (size, batch); grid: (size, grid); k: scalar
            # print(y_eval.device, x_eval.device, grid.device)
        ...
        self.mask = torch.ones(self.size).to(self.device)

        self._grid = torch.einsum('i,j->ij', torch.ones(self.size), torch.linspace(grid_range[0], grid_range[1], steps=grid_number + 1)).to(self.device)  #shape:(in*out, grid_number+1)  range[-1,+1]  distribution:evenly
        self.coef = torch.nn.Parameter(curve2coef((torch.rand(self.size, self._grid.shape[1]).to(self.device)-1/2)*noise_scale/grid_number,  self._grid, self._grid, k)).to(self.device)  #shape:(size, coef)
        self.scale_base = torch.nn.Parameter(torch.ones(self.size, ) * scale_base).requires_grad_(sb_trainable).to(self.device)
        self.scale_spline = torch.nn.Parameter(torch.ones(self.size, ) * scale_spline).requires_grad_(sp_trainable).to(self.device)
    ...
    def forward(self, x): #x:[-1,in_dim]
        ...
        i = torch.einsum('ij,k->ikj', x, torch.ones(self.out_dim).to(self.device)).reshape(x.shape[0], self.size).permute(1,0)  # x: shape(batch, in_dim) => shape(out_dim*in_dim, batch)  #engineering: optimizable
